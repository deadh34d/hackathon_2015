...
class Home
{
    ...
    {
        $db = Database::connect();
        $query = 'SELECT * FROM homes WHERE homes.id = :id';
        $statement = $db->prepare($query);
    ...
    public function get_home_coords($id)
    {
        $db = Database::connect();
        $query = 'SELECT `x_coord`, `y_coord` FROM homes WHERE homes.id = 1';
        $statement = $db->prepare($query);
        $statement->bindValue(":id", $id);
        $statement->execute();
        $result = $statement->fetch(PDO::FETCH_ASSOC);
    ...
    public function add_home($sd, $lot, $sold, $street_num,
    ...
    public function add_home_from_array($input_array)
    {
        $db = Database::connect();
    ...
    public function edit_home_from_array($input_array, $id)
    {
        $db = Database::connect();
    ...
    public function search_homes($filter)
    {
        $db = Database::connect();
        $filter_query = $filter['price'] . " AND " . $filter['sqft']
            . " AND " . $filter['bed'] . " AND " . $filter['location'];
        $query = "SELECT * FROM homes WHERE $filter_query";
        $statement = $db->prepare($query);
        $statement->execute();
        $result = $statement->fetchAll(PDO::FETCH_ASSOC);
        $statement->closeCursor();
    ...
    public function get_realtor($id) {
        $db = Database::connect();
        $query = "SELECT name, url FROM realtors, homes
                  WHERE homes.realtor_id = realtors.realtor_id
                  AND homes.id = :id";
        $statement = $db->prepare($query);
        $statement->bindValue(":id", $id);
        $statement->execute();
